...
const getCartProducts = async (req, res) => {
  ...
  try {
    ...
    if (cart) {
      ...
      return res
        .status(200)
        .json({ success: true, message: 'cart found', cart, totalBill });
    } else {
      return res
        .status(504)
        .json({ success: false, message: 'finding cart failed' });
    }
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    try {
      const newCart = await Cart.create({
        userId,
        products: { productId: id, name, image: products.image, quantity },
        bill: quantity * price,
      });
      return res
        .status(201)
        .json({ success: true, message: 'Added to cart', newCart });
    } catch (err) {
      console.log(err);
